...
async function ddFetchJson(params, label = "DD backend request") {
  try {
    const localResponse = await fetch("/api/dd", {
      method: "POST",
      credentials: "same-origin",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify(params)
    });
    const localData = await ddReadJson(localResponse);
    return { url: "/api/dd", response: localResponse, data: localData, label };
  } catch (error) {
    if (["login", "signup"].includes(String(params.mode || "").toLowerCase())) throw error;
    console.warn("Local DD backend unavailable, falling back to legacy endpoint.", error);
  }
